...
upload: dnc-bootloader.c32
	rsync -z dnc-bootloader.c32 numascale:/net/numastore/tftpboot/dnc-bootloader-$(USER).c32
...
%.o: %.c $(syslinux_dir)/com32/samples/Makefile
	(rm -f $@ && cd $(syslinux_dir)/com32/samples && make LD="/bin/foo" CC="g++ -fno-rtti -fpermissive -Wall -Wextra -ftrapv" $(CURDIR)/$@ NOGPL=1)
