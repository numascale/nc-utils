...
{
    const char *phyname = _get_linkname(linkno);
    u32 stat, elog, ctrl, tries;

    tries = 0;
again:    
    // Enable the dynamic frequency drift control (only FPGA has this so far)
    ctrl = dnc_read_csr(0xfff0, H2S_CSR_G0_PHYXA_LINK_CTR + ((linkno-1)<<6));
    if (!(ctrl & (1<<12)) || !(ctrl & (1<<13))) {
        dnc_write_csr(0xfff0, H2S_CSR_G0_PHYXA_LINK_CTR + ((linkno-1)<<6), ctrl | (1<<13) | (1<<12));
    }

    stat = dnc_read_csr(0xfff0, H2S_CSR_G0_HSSXA_STAT_1 + ((linkno-1)<<6));
    ...
    stat = dnc_read_csr(0xfff0, H2S_CSR_G0_PHYXA_LINK_STAT + ((linkno-1)<<6));
    elog = dnc_read_csr(0xfff0, H2S_CSR_G0_PHYXA_ELOG + ((linkno-1)<<6));
...
{
    const char *linkname = _get_linkname(linkno);
    u32 initst, error_count;
    const u32 fatal_mask0 = 0x0;
    const u32 fatal_mask1 = 0x0;

    if (dnc_raw_read_csr(0xfff0 + linkno, LC3_CSR_INIT_STATE, &initst) != 0) {
        return -1;
    }
    if (dnc_raw_read_csr(0xfff0 + linkno, LC3_CSR_ERROR_COUNT, &error_count) != 0) {
        return -1;
    }
    ...
    if (error_count != 0) {
        u32 elog0, elog1;
        if (dnc_raw_read_csr(0xfff0 + linkno, LC3_CSR_ELOG0, &elog0) != 0) {
            return -1;
        }
        if (dnc_raw_read_csr(0xfff0 + linkno, LC3_CSR_ELOG1, &elog1) != 0) {
            return -1;
        }
        dnc_write_csr(0xfff0 + linkno, LC3_CSR_ERROR_COUNT, 0);
        dnc_write_csr(0xfff0 + linkno, LC3_CSR_ELOG0, 0);
        dnc_write_csr(0xfff0 + linkno, LC3_CSR_ELOG0, 0);
...
//                  dnc_read_csr(0xfff0 + linkno, LC3_CSR_CONFIG3) | (1<<12)); // Set fatal2dead
    ...
    /* 1. Disable table routing */
    dnc_write_csr(0xfff0 + linkno, LC3_CSR_ROUT_CTRL, 3 << 14); // ROUT_CTRL.rtype = 2'b11 (all)
	...
	printf(".");
        ...
        for (offs = 0; offs < 16; offs++) {
            dnc_write_csr(0xfff0 + linkno, LC3_CSR_ROUT_LCTBL00  + (offs<<2), ltbl[(chunk<<4)+offs]);
            dnc_write_csr(0xfff0 + linkno, LC3_CSR_ROUT_BXTBLL00 + (offs<<2), rtbll[(chunk<<4)+offs]);
            dnc_write_csr(0xfff0 + linkno, LC3_CSR_ROUT_BLTBL00  + (offs<<2), rtblm[(chunk<<4)+offs]);
            dnc_write_csr(0xfff0 + linkno, LC3_CSR_ROUT_BXTBLH00 + (offs<<2), rtblh[(chunk<<4)+offs]);
        }
    }
    printf("\n");

    /* 3. Enable table routing */
    dnc_write_csr(0xfff0 + linkno, LC3_CSR_ROUT_CTRL, 1 << 14); // ROUT_CTRL.rtype = 2'b01 (table routing)

    if (dnc_raw_read_csr(0xfff0 + linkno, LC3_CSR_ERROR_COUNT, &error_count2) != 0) {
        return -1;
    }

    if (error_count1 != error_count2) {
        printf("Errors while initializing  LC3%s (%d != %d)\n",
               _get_linkname(linkno), error_count1, error_count2);
        return -1;
    }
