...
typedef struct ddr1_spd_eeprom_s {
        ...
        uint8_t nrows;       /*  5 Number of DIMM Banks */
        uint8_t dataw_lsb;   /*  6 Data Width of this assembly */
        uint8_t dataw_msb;   /*  7 ... Data Width continuation */
        ...
        uint8_t min_delay;   /* 15 for Back to Back Random Address */
        uint8_t burstl;      /* 16 Burst Lengths Supported */
        uint8_t nbanks;      /* 17 # of Banks on SDRAM Device */
        uint8_t cas_lat;     /* 18 CAS# Latencies Supported */
        uint8_t cs_lat;      /* 19 CS# Latency */
        uint8_t write_lat;   /* 20 Write Latency (aka Write Recovery) */
        uint8_t mod_attr;    /* 21 SDRAM Module Attributes */
        uint8_t dev_attr;    /* 22 SDRAM Device Attributes */
        uint8_t clk_cycle2;  /* 23 Min SDRAM Cycle time @ CL=X-0.5 */
        uint8_t clk_access2; /* 24 SDRAM Access from
                                         Clk @ CL=X-0.5 (tAC) */
        uint8_t clk_cycle3;  /* 25 Min SDRAM Cycle time @ CL=X-1 */
        uint8_t clk_access3; /* 26 Max Access from Clk @ CL=X-1 (tAC) */
        ...
        uint8_t bank_dens;   /* 31 Density of each bank on module */
        uint8_t ca_setup;    /* 32 Addr + Cmd Setup Time Before Clk */
        uint8_t ca_hold;     /* 33 Addr + Cmd Hold Time After Clk */
        uint8_t data_setup;  /* 34 Data Input Setup Time Before Strobe */
        uint8_t data_hold;   /* 35 Data Input Hold Time After Strobe */
        uint8_t res_36_40[5];/* 36-40 reserved for VCSDRAM */
        ...
        uint8_t res_46;      /* 46 Reserved */
        uint8_t dimm_height; /* 47 DDR SDRAM DIMM Height */
        uint8_t res_48_61[14]; /* 48-61 Reserved */
        uint8_t spd_rev;     /* 62 SPD Data Revision Code */
        uint8_t cksum;       /* 63 Checksum for bytes 0-62 */
        uint8_t mid[8];      /* 64-71 Mfr's JEDEC ID code per JEP-106 */
...
extern int ddr1_spd_check(const ddr1_spd_eeprom_t *spd);
extern int ddr2_spd_check(const ddr2_spd_eeprom_t *spd);
extern int ddr3_spd_check(const ddr3_spd_eeprom_t *spd);
